...
2nd is if this is call form register
3rd is userinfo use for regisration
4th is if this is call from chat
5th is one of the userid of conversation
*/
...
const checkConversationExist = function(msg, getChat = false) { //check if the conversation between the 2 user exist in db
  if (msg.from == msg.to) { //left as == just in case type different
...
//similar to getTweets, only exist because escpaing add quote which broke query
...
//take in the userName and tweetID and check owner
...
//get chatlog with conversation ID, order by most recent
